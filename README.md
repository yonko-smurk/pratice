...
```
> "768 by default — that's what `nomic-embed-text` produces — but the dimension is driven by the `EMBED_DIM` env var, so the same column can realign to 1024 for `mxbai-embed-large` or 1536 for OpenAI's `text-embedding-3-small` without touching code: just bump the env, run a migration, re-ingest. The `null=True` lets articles be saved before chunks are embedded — useful during ingest — and `token_count` is stored alongside so the prompt-builder can budget tokens without re-tokenising. Redis sits beside Postgres as the Channels message broker and the session cache."
